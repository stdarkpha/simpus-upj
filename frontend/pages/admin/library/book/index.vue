    ...
    <Tabs :default-value="tabs.toString()">
          ...
          <div class="text-2xl flex items-center justify-between my-4 font-semibold">
            <div>
              <h3 class="text-lg font-medium">
                Book List ({{ adminStore.data_books.length }})
              </h3>
      ...
      <TabsContent value="2">
        <div class="text-2xl flex items-center justify-between my-4 font-semibold">
          <div>
            <h3 class="text-lg font-medium">
              Book Category ({{ adminStore.data_books_category.length }})
            </h3>
            <p class="text-sm text-muted-foreground">
          ...
          <Sheet @update:open="(value) => {
            ...
            <SheetContent>
              ...
              <Form @refresh="() => { id = null; open = false; useRouter().push('/admin/library/book?tab=2'); }"
                :id="id" />
              <SheetDescription>
              </SheetDescription>
            </SheetContent>
...
await adminStore.fetchBooks(20)
