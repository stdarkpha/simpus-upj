            ...
            >
               <SheetContent class="gap-0">
                  <SheetHeader class="border">
                     <SheetTitle>Detail {{ lend_detail.data.status == "pending" ? "Peminjaman" : "Pengembalian" }}</SheetTitle>
                  </SheetHeader>
                  <div class="grow py-2 relative overflow-y-auto overflow-x-hidden">
                     <div class="p-4 pt-0 relative">
                        <div class="flex items-center justify-between mb-2">
                           ...
                           <div class="text-xs capitalize bg-orange-600 text-orange-100 px-2 py-1.5 rounded">{{ lend_detail.data.status }}</div>
                        </div>
                        ...
                        <div class="flex items-center gap-4 justify-between">
                           ...
                           <div class="text-sm flex-1 capitalize">
                              <span class="block">{{ lend_detail.data.user.role == "mahasiswa" ? "NIM" : "NIDN" }}</span>
                           ...
                           <div v-for="data in lend_detail.data.items" class="mb-4">
                              <div class="flex items-center gap-4">
                                 ...
                                 <div>
                                    ...
                                    <p class="text-sm text-gray-500 capitalize">Kategori: {{ data.book.category.name }}</p>
                                 </div>
                              </div>
                           </div>
                        </div>
                  ...
                  <div class="flex gap-2 p-2 w-full z-50">
                     ...
                     <motion.button @click="setStatus('reject')" :while-press="{ scale: 0.8 }" class="flex-1 py-2 rounded-md bg-red-100 border border-red-500 text-red-600">Reject</motion.button>
                     <motion.button @click="setStatus('approve')" :while-press="{ scale: 0.8 }" class="flex-1 py-2 rounded-md bg-green-100 border border-green-500 text-green-600">Approve</motion.button>
                  </div>
               </SheetContent>
            </Sheet>
            <div>
