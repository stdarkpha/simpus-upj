<template>
    <div>
        ...
        <div class="px-4">
            <motion.div v-for="item in history_data?.data" :layout-id="`history-${item.id}`"
                ...
                <motion.div :initial="false" :animate="{ height: 40 }"
                    ...
                    <div class="flex gap-2 px-4 py-2" v-for="(data, index) in item.compact">
                        ...
                        <div v-if="index == 0 && item.compact.length > 1" class="text-sm shrink-0">
        ...
        <Teleport to="body">
            <motion.div v-if="selected_item" @click="selected_item = null; showqrcode = false;"
                :initial="{ bottom: '-20%', opacity: 0 }"
                :animate="{ bottom: '10%', opacity: 1, transition: { delay: .5, duration: 0.4 } }"
            ...
            <motion.div v-if="selected_item" :initial="{ background: 'rgba(231, 0, 11 0)' }"
                ...
                <motion.div class="bg-white pt-4 pb-12 rounded-md relative" v-on-click-outside="() => {
                    selected_item = null
                    showqrcode = false;
                }">
                    <motion.div :initial="{ y: '-100%', opacity: 0 }"
                        :animate="{ y: '-50%', opacity: 1, transition: { delay: .5, duration: 0.4 } }"
                        class="w-10 h-10 rounded-full left-1/2 z-10 -translate-x-1/2 absolute top-0 bg-red-600">
                    </motion.div>
                    <motion.div :initial="{ y: '100%', opacity: 0 }"
                        :animate="{ y: '50%', opacity: 1, transition: { delay: .5, duration: 0.4 } }"
                    ...
                    <div class="relative flex items-center justify-center">
                        <div :class="showqrcode ? 'rotate-y-180' : ''"
                            class="transition-all duration-500 backface-hidden">
                            <motion.div :layout-id="`history-${selected_item.id}`"
                                class="container p-4 flex flex-col gap-2 rounded-md bg-white">
                                <div class="flex items-baseline-last justify-between w-full">
                                    <div class="font-bold uppercase">
                                ...
                                <motion.div :initial="{ height: 50 }"
                                    :animate="{ height: 160, transition: { duration: 0.4, delay: .2 } }"
                                ...
                                <div class="flex items-baseline-last justify-between w-full">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span class=" font-semibold text-red-600">{{
                                            formatDate(selected_item.created_at)
                                        }}</span>
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-1 items-end">
                                        <span class="text-gray-500 text-xs">Batas Peminjaman</span>
                                        <span class=" font-semibold text-red-600">{{
                                            formatDate(selected_item.return_date)
                                        }}</span>
                                    </div>
                                </div>
                            </motion.div>
                        ...
                        <div :class="!showqrcode ? 'rotate-y-180' : 'rotate-y-0 '"
                            class="absolute w-auto h-full transition-all duration-500 pb-10 pt-4 backface-hidden">
                            <h2 class="mb-1 px-4">Gunakan QR berikut untuk mengambil buku</h2>
                    ...
                    <div class="px-4">
                        <motion.button :initial="{ opacity: 0, scale: 0 }" @click="showqrcode = !showqrcode"
                            :animate="{ opacity: 1, scale: 1, transition: { duration: 0.4, delay: .5 } }"
