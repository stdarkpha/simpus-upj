        ...
        <header>
            <div class="container p-4 flex flex-col gap-8">
                ...
                <div class="bg-white w-full flex items-center justify-center rounded-md px-3">
                    ...
                    <input type="text" placeholder="Cari buku berdasarkan judul" v-model="query"
        ...
        <section class="mb-8 mt-4">
            ...
            <div v-else>
                <div v-if="search_result?.data?.length > 0" class="w-full flex flex-col gap-4">
                    <motion.div :initial="{ opacity: 0, y: 100 }"
                        :animate="{ opacity: 1, y: 0, transition: { duration: 0.6 } }"
                        class="w-full h-48 flex flex-row group items-end p-4 rounded-md gap-3 relative z-10"
                        v-for="(item) in search_result?.data" @click="item_active = item">
                ...
                <motion.div :initial="{ opacity: 0, y: 100 }" :animate="{ opacity: 1, y: 0 }"
                    v-else-if="search_result?.data?.length === 0 || search_result?.success === false"
                    class="w-full py-24 text-gray-600 gap-2 flex flex-col items-center justify-center">
                    <Icon icon="mdi:alert-circle-outline" class="text-6xl" />
                    Tidak ada buku yang ditemukan
                </motion.div>

                <motion.div :initial="{ opacity: 0, y: 100 }"
                    :animate="{ opacity: 1, y: 0, transition: { duration: 0.6 } }" v-else
                    class="w-full py-24 gap-4 text-gray-600 flex flex-col items-center justify-center">
                    <Icon icon="mingcute:loading-3-line" class="text-6xl animate-spin " />
                    Mencari Buku...
                </motion.div>

                <UserDetail type="category" v-if="item_active" :item_active="item_active"
                    @close="() => item_active = null" />
            </div>
        </section>
    </div>
</template>
