...
<template>
  <Sidebar class="z-50">
    <SidebarHeader>
      <NavHeader />
      <Search v-if="useSanctumUser<UserData>().value?.role_id == 1" />
    </SidebarHeader>
    <SidebarContent>
      <SidebarContent>
        <SidebarGroup v-for="(nav, indexGroup) in navMenu" :key="indexGroup">
          ...
          <template v-for="(item, index) in nav.items" :key="index">
            <template v-if="item.children">
              <LayoutSidebarNavGroup v-if="!item.only || item.only.includes(useSanctumUser<UserData>().value?.role_id)"
                :item="item" />
            </template>
            <template v-else>
              <LayoutSidebarNavLink v-if="!item.only || item.only.includes(useSanctumUser<UserData>().value?.role_id)"
                :item="item" />
            </template>
          </template>
        </SidebarGroup>
    ...
    <SidebarFooter>
      <SidebarNavFooter :user="useSanctumUser<UserData>().value ?? { name: '', email: '', avatar: null }" />
    </SidebarFooter>
  </Sidebar>
</template>